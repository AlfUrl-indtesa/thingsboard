...
const menuFilters = new Map<MenuId, MenuFilter>([
  [
    MenuId.edges,
    (authState) => authState.edgesSupportEnabled,
  ],
  [
    MenuId.edge_management,
    (authState) => authState.edgesSupportEnabled,
  ...
  [
    ...
    [
      ...
      },
      {
        ...
        places: [MenuId.reports, MenuId.data_export],
      },
    ],
  ],
  [
...
export const buildUserHome = (
  authState: AuthState,
  availableMenuSections: MenuSection[],
): Array<HomeSection> => {
  const references = defaultHomeSectionMap.get(authState.authUser.authority);
  return (references || []).map((ref) =>
    homeReferenceToHomeSection(availableMenuSections, ref)
  ).filter((section) => !!section);
};

const referenceToMenuSection = (
  authState: AuthState,
    ...
    if (section) {
      ...
      if (reference.pages?.length) {
        result.pages = reference.pages.map((page) =>
          referenceToMenuSection(authState, page)
        ).filter((page) => !!page);
      }
      return result;
    } else {
...
const filterMenuReference = (
  ...
  if (filter) {
    if (filter(authState)) {
      if (reference.pages?.length) {
        if (
          reference.pages.every((page) => !filterMenuReference(authState, page))
        ) {
          return false;
        }
      }
...
const homeReferenceToHomeSection = (
  availableMenuSections: MenuSection[],
  reference: HomeSectionReference,
): HomeSection | undefined => {
  const places = reference.places.map((id) =>
    availableMenuSections.find((m) => m.id === id)
  ).filter((p) => !!p);
  if (places.length) {
    return {
      name: reference.name,
